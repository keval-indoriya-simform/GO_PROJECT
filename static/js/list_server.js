$(document).ready(async function(){
    let retrieveModelData = await retrieveModel(`http://localhost:8080/api/v1/servers?page=1`)
    ...
    paginationLoad(retrieveModelData["total_pages"], `http://localhost:8080/api/v1/servers?page=`)
    ...
    $("#delete-btn").click(function () {
        delete_data("http://localhost:8080/api/v1/servers?server_id="+list.toString(),
            `http://localhost:8080/network-management-solutions/list/servers`)
    })

    $('#serverModal').on('show.bs.modal', async function() {
        let dataId = $("#serverModal").attr("data-id");
        console.log(dataId)
        let serverResponse = await fetch('http://localhost:8080/api/v1/servers?server_id=' + dataId);
        ...
        update_button.click(async function(){
            if(checkExpirationDate(document.getElementById("validationPurchaseDate").value,document.getElementById("validationExpirationDate").value)) {
                ...
                await patchModel(
                    ...
                    "http://localhost:8080/api/v1/servers?server_id=" + dataId,
                    `http://localhost:8080/api/v1/servers?page=`
                )
            } else{
