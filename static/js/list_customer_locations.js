$(document).ready(async function(){
    let retrieveModelData = await retrieveModel(`http://192.168.49.2:31471/api/v1/customer-locations?select_column=customer_locations.customer_location_id&append_select=true&page=1`)
    ...
    paginationLoad(retrieveModelData["total_pages"], `http://192.168.49.2:31471/api/v1/customer-locations?select_column=customer_locations.customer_location_id&append_select=true&page=`)
    ...
    $("#delete-btn").click(function () {
        delete_data("http://192.168.49.2:31471/api/v1/customer-locations?customer_location_id="+list.toString(),
            `http://192.168.49.2:31471/network-management-solutions/list/customer-locations`)
    })

    $('#modalCustomerLocation').on('show.bs.modal', async function() {
        let dataId = $("#modalCustomerLocation").attr("data-id");
        console.log(dataId)
        let customerLocationResponse = await fetch('http://192.168.49.2:31471/api/v1/customer-locations' +
        ...
        update_button.click(async function(){
            await patchModel(
                ...
                "http://192.168.49.2:31471/api/v1/customer-locations?customer_location_id="+dataId,
                `http://192.168.49.2:31471/api/v1/customer-locations?select_column=customer_locations.customer_location_id&append_select=true&page=`
            )
        });
    });
