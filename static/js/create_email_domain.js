...
$(document).ready(async function() {
    ...
    const customerLocationResponse = await fetch('http://localhost:8080/api/v1/customer-locations?select_column=customer_locations.customer_location_id,customer_locations.name');
    const customerLocationData = await customerLocationResponse.json();
    // console.log(customerLocationData)
    const emailAccountTypesResponse=await fetch('http://localhost:8080/api/v1/email-domains/email-account-types')
    ...
    create_button.click(async function(){
        ...
        await postModel(
            ...
            "http://localhost:8080/api/v1/email-domains"
        )
    });
