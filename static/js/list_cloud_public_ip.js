$(document).ready(async function(){
    let retrieveModelData = await retrieveModel(`http://localhost:8080/api/v1/cloud-public-ips?page=1`)
    ...
    paginationLoad(retrieveModelData["total_pages"], `http://localhost:8080/api/v1/cloud-public-ips?page=`)
    ...
    $("#delete-data-btn").click(function () {
        delete_data("http://localhost:8080/api/v1/cloud-public-ips?cloud_public_ip_id="+list.toString(),
            `http://localhost:8080/network-management-solutions/list/cloud-public-ips` )
    })

    $('#cloudPublicIpModal').on('show.bs.modal', async function() {
        let dataId = $("#cloudPublicIpModal").attr("data-id");
        console.log(dataId)
        let cloudPublicIpResponse = await fetch('http://localhost:8080/api/v1/cloud-public-ips?cloud_public_ip_id=' + dataId);
        let cloudPublicIpData = await cloudPublicIpResponse.json()
        console.log(cloudPublicIpData)
        let postForwardResponse = await fetch('http://localhost:8080/api/v1/cloud-private-ips?set_limit=false');
        ...
        update_button.click(async function(){
            await patchModel(
                ...
                "http://localhost:8080/api/v1/cloud-public-ips?cloud_public_ip_id="+dataId,
                `http://localhost:8080/api/v1/cloud-public-ips?page=`
            )
        });
    });
